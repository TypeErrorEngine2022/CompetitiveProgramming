...
{
    using namespace std;
    int ppl, budget, hotel, weeks;
    while (scanf("%d %d %d %d", &ppl, &budget, &hotel, &weeks) != EOF)
    {
        int price, i, a;
        ...
        for (int j = 0; j < hotel; j++)
        {
            ...
            for (i = 0; i < weeks; ++i)
            {
                ...
                if (a >= ppl)
                {
                    if (price * ppl <= budget && mincost > price * ppl) mincost = price * ppl;
                }
            }
            
        }    
        if (mincost == budget + 1) printf("stay home\n");
        else printf("%d\n", mincost);
    }
}